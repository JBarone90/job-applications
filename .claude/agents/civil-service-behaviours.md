...
- Real examples to draw from, in priority order: `examples/*.md` (full write-ups, tagged by criteria/behaviour — search these first via Grep on the `tags:` frontmatter), then `cv/experience.tex`/`cv/education.tex` (compressed bullets), then anything the user describes in conversation.
...
3. Grep `examples/` for a tag matching each behaviour; fall back to `cv/experience.tex`/`cv/education.tex` or the user's own account if nothing matches. If nothing genuine fits, say so — do not invent one.
